...
## 매 작업 시작 시
1. `PIPELINE.md`를 읽는다 (드라이브 확인→반영→배포 절차, 실전 함정들 — 작업 디렉토리가
   여러 개일 수 있다는 것도 여기 0장에 있음, 꼭 확인)
2. `HANDOFF.md`를 읽는다 (여행 사실관계, 되돌리면 안 되는 결정, 버그 이력, 데이터, 톤)
3. `.claude/CLAUDE.md`의 코드 규칙을 확인한다
4. `git fetch origin main && git log origin/main -5 --oneline`으로 다른 세션이
   이미 커밋한 게 없는지 확인한다
5. 무엇을 바꿀지, 왜 바꾸는지 한 줄로 말하고 시작한다
...
## 절대 규칙
- **되돌리지 말 것**: 레고랜드 제외, 포켓몬센터/파르코 6층 제외, SVG/개념도 지도 부활,
  특정 맛집 고정(후보 형태 유지). "첫날 도톤보리 제외"는 2026-09-21 사용자가 직접
  되돌렸다(원더크루즈가 첫날에 들어감) — 최신 사항은 `HANDOFF.md`의 "설계상 뺀 것" 확인
...
아이 동반 관점의 실질적 경고 우선 — 단, **유모차 여부는 확정된 게 아니니 HANDOFF.md에서
최신 상태 확인 후 서술할 것** (없다고 단정하지 말 것, 최근 뒤집힌 이력 있음).
