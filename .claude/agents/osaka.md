...
## 절대 규칙
- **되돌리지 말 것**: 레고랜드 제외, 포켓몬센터/파르코 6층 제외, 첫날 도톤보리 제외, SVG 개념도, 특정 맛집 고정(후보 형태 유지)
...
## 변경 후
1. 로컬에서 브라우저로 두 페이지(일정 / 맛집 지도) 모두 눈으로 확인. 페이퍼 확인 금지
2. 지도는 실제 지도·개념도 둘 다, 난바·나라 둘 다 확인
3. 커밋 메시지는 무엇을·왜. 푸시 계정은 `eungil`
4. 푸시 후 https://eungil.github.io/japantrip/ 에서 반영 확인 (1~2분)
...
## 다음 할 일 후보 (HANDOFF 7장, 우선순위 순)
1. 배포본 지도 렌더 확인
2. 단일 HTML 분리 (index.html + style.css + app.js + data.json)
3. 일정 데이터 구조화 → 템플릿 렌더링
4. Service Worker 오프라인 대응
5. `navigator.geolocation` 현재 위치 표시
6. 일정 시각 변경 시 뒤 일정 자동 밀림 계산
